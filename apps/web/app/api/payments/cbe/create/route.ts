...
export const POST = withApiHandler(async (req: NextRequest) => {
  ...
  // Throws ProviderNotConfiguredError (→ 503): PENDING OFFICIAL CBE MERCHANT/API SPECIFICATION.
  const payment = await createDeposit({ userId: current.sub, provider: "CBE", amount });
