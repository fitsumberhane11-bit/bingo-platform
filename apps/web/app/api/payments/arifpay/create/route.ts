...
export const POST = withApiHandler(async (req: NextRequest) => {
  ...
  // Throws ProviderNotConfiguredError (→ 503): NOT CONNECTED, no ArifPay merchant credentials yet.
  const payment = await createDeposit({ userId: current.sub, provider: "ARIFPAY", amount });
