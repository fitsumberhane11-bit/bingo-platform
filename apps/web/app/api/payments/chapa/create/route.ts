...
export const POST = withApiHandler(async (req: NextRequest) => {
  ...
  // Throws ProviderNotConfiguredError (→ 503): NOT CONNECTED, no Chapa merchant credentials yet.
  const payment = await createDeposit({ userId: current.sub, provider: "CHAPA", amount });
