...
import { resumeGame } from "@/lib/game/engine";
...
export const POST = withApiHandler(async (_req: Request, { params }: { params: { gameId: string } }) => {
  const ctx = await requireApiPermission(PERMISSIONS.GAME_RESUME);
  const game = await resumeGame(params.gameId, ctx.userId);
